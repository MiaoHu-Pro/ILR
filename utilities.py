...
def read_ent_rel_2id(data_id_path):
    f = open(data_id_path)
...
class Enti(object):
    def __init__(self, _id, _symbol,_project, _label, _type,_status, _mention, _neighbours, _entity2vec, _create_time, _entity_des_word_list=None):
        self.id = str(_id)
        ...
        self.created_time = _create_time
...
def construct_entity_des(entity_project, entity_type, entity_name, entity_mention, current_entity_des_using_name):
    """
    given a entity , which was transformed into a word vector
    """

    _entity_type = clean(entity_type)
    _entity_project = clean(entity_project)

    if entity_name == 'None' or len(entity_name) == 0 or entity_mention == np.nan:
        _entity_name = "No name"
    else:

        _entity_name = entity_name.split(" ")
    _entity_mention = []
    if entity_mention == 'None' or entity_mention == np.nan:
        _entity_mention = _entity_name
    else:

        _entity_mention = entity_mention.split(" ")

    entity_description_list = []
    entity_description_list += _entity_project
    entity_description_list += [',']
