...
def do_relation_tail_prediction(args, model, tokenizer, processor, label_list, device):
    ...
    print("head_with_relations_and_tail_candidates", head_with_relations_and_tail_candidates)
    ...
    file_prefix = str(args.output_dir) + str(args.data_dir[7:]) + "_batch_size_" + str(
        ...
        args.negative) + "_model_type_" + str(args.model_type) + "_model_name_" + str(
        args.model_name_or_path) + "_" + str(args.test_file) + "_" + str(
        random.randint(0, 10000000)) + "_max gap_" +str(args.max_time_gap)+"_get_candidates_rand4morethreshold_"
...
def do_triple_classification(args, model, tokenizer, processor, label_list, device):
    # test_triples = processor.get_test_triples_for_link_prediction(args.data_dir, args.test_file)  # load test sample

    model.to(device)
    _examples = processor.get_test_example_for_triple_classification(args.data_dir, args.negative)

    _features = processor.to_convert_examples_to_features(
        _examples, label_list, args.max_seq_length, tokenizer)
    ...
    all_label_ids = torch.tensor([f.label_id for f in _features], dtype=torch.long)
    eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids)
    ...
    write_true_ids_and_preds(args, _examples, all_label_ids, preds)


def do_link_prediction(args, model, tokenizer, processor, label_list, device):
    head_with_relations_and_tail_candidates = None
    tail_with_relations_and_head_candidates = None
    entityType_with_id = None
    relationid_with_name = None
    test_entity_index_with_type = None
    entityid_with_created_time = None
    training_entity_with_type = None
    if args.which_test == 'test':
        head_with_relations_and_tail_candidates, tail_with_relations_and_head_candidates, entityType_with_id, \
        relationid_with_name, training_entity_with_type, test_entity_index_with_type, entityid_with_created_time = processor.get_triple_type_info()

    # elif args.which_test == 'valid':
    #     head_with_relations_and_tail_candidates, tail_with_relations_and_head_candidates, entityType_with_id, \
    #     relationid_with_name, test_entity_index_with_type = processor.get_valid_triple_type_info()
    ...
    all_triples = train_triples + dev_triples + total_test_triples
    ...
    file_prefix = str(args.output_dir) + str(args.data_dir[7:]) + "_per_gpu_train_batch_size_" + str(
        args.per_gpu_train_batch_size) + "_lr_" + str(
        args.learning_rate) + "_max_seq_" + str(args.max_seq_length) + "_num_train_epochs_" + str(
        args.num_train_epochs) + "_negative_" + str(
        args.negative) + "_model_type_" + str(args.model_type) + "_model_name_or_path_" + str(
        args.model_name_or_path) + "_flag_" + str(args.random_flag) + "_rand_num_candidates_" + str(
        args.random_candidate_tails) + "_" + str(args.test_file)
    ...
    for test_triple in sub_test_triples:
        ...
        # tail_type = test_entity_index_with_type[tail]
        # print("tail_type :", tail_type)
        # relation_name = relationid_with_name[relation]
        # print("relation_name :", relation_name)
        # candidate_head_entity_type_list = tail_with_relations_and_head_candidates[tail_type][relation_name]
        # print("candidate_head_entity_type_list :", candidate_head_entity_type_list)
        # candidate_head_entity_list = []
        #
        # for entity_type in candidate_head_entity_type_list:
        #     _list_type_with_id = list(entityType_with_id[entity_type])
        #     candidate_head_entity_list += _list_type_with_id
        #
        # print("candidate_head_entity_list: ", len(candidate_head_entity_list))
        # head_corrupt_list = [test_triple]
        #
        # for corrupt_ent in candidate_head_entity_list:
        #     if corrupt_ent != head:
        #         tmp_triple = [str(corrupt_ent), str(tail), str(relation)]
        #         tmp_triple_str = ''.join(tmp_triple)
        ...
        #
        # eval_examples = processor.to_create_test_examples_for_link_prediction(head_corrupt_list, args.en_from_set)
        #
        ...
        if "train" in args.test_file:
            head_type = training_entity_with_type[head]
        else:
            head_type = test_entity_index_with_type[head]

        relation_name = relationid_with_name[relation]
        candidate_tail_entity_type_list = head_with_relations_and_tail_candidates[head_type][relation_name]
        candidate_tail_entity_list = []
        ...
        # randomly choose M (random_candidate_tails) candidates for test
        if args.random_flag != 'false':
            if len(candidate_tail_entity_list) >= args.random_candidate_tails:
                candidate_tail = sample(candidate_tail_entity_list, args.random_candidate_tails)
            else:
                candidate_tail = candidate_tail_entity_list
        else:
            # select the tail candidates according to the gap-time, 5472 for MangoDB
            for _tail in candidate_tail_entity_list:
                tail_with_time = entityid_with_created_time[_tail]
                gap_hour = gap_time(head_with_time, tail_with_time)
                if 0 < gap_hour <= args.gap_time:
                    candidate_tail.append(_tail)

        logger.info("after filet, the number of candidate_tails:{0}".format(len(candidate_tail)))
        for corrupt_ent in candidate_tail:
            if corrupt_ent != tail:
                tmp_triple = [str(head), str(corrupt_ent), str(relation)]
        ...
        eval_examples = None
        eval_examples = processor.to_create_test_examples_for_link_prediction(tail_corrupt_list, args.en_from_set)
        eval_features = processor.to_convert_examples_to_features(
            eval_examples, label_list, args.max_seq_length, tokenizer)
        ...
        f.write(str(tail_corrupt_list[0]) + "-->" + str(len(candidate_tail)) + "-->" + str(rank2) + '\n')
    ...
    # write the result to file:
    f = open(str(args.output_dir) + 'link_predict_results.txt', 'a')
    f.write(file_prefix + " :\n")
...
def do_isolated_node_detection(args, model, tokenizer, processor, label_list, device):
    head_with_relations_and_tail_candidates, tail_with_relations_and_head_candidates, entityType_with_id, \
    relationid_with_name, training_entity_with_type, test_entity_index_with_type, entityid_with_created_time = processor.get_triple_type_info()
    ...
    test_isolated_nodes = processor.get_test_isolated_nodes(args.data_dir, args.test_file)  # load test sample
    ...
    file_prefix = str(args.output_dir) + str(args.data_dir[7:]) + "train_batch_size_" + str(
        args.per_gpu_train_batch_size) + "_lr_" + str(
        args.learning_rate) + "_seq_length_" + str(args.max_seq_length) + "train_epochs_" + str(
        args.num_train_epochs) + "_negative_" + str(
        args.negative) + "_model_type_" + str(args.model_type) + "_model_name_" + str(
        args.model_name_or_path) + "_" + str(args.test_file)

    pre_labels = []
    using_relation = None
    if str(args.data_dir[7:]) == "MongoDB_data":
        using_relation = ['related to', 'depends on', 'duplicates', 'included in Initiative']
    if str(args.data_dir[7:]) == "hadoop_77K":
        using_relation = ['relates to', 'depends upon', 'duplicates', 'contains']
    if str(args.data_dir[7:]) == "RedHat_data":
        using_relation = ['relates to', 'depends on', 'duplicates', 'incorporates']

    logger.info("using_relation : {0}".format(using_relation))

    for test_nodes in test_isolated_nodes:

        logger.info("***** Running data preparation *****")
        logger.info("test_nodes:{0}".format(test_nodes))
        node_name = test_nodes[0]
        head = test_nodes[1]
        ...
        _temp_relation = head_with_relations_and_tail_candidates[head_type].keys()

        tail_corrupt_list = []
        _each_relation_candidates = []
        for _relation in _temp_relation:
            if _relation not in using_relation:
                continue

            _relation_id = [k for k, v in relationid_with_name.items() if v == _relation][0]

            _tail_corrupt_list = []
            candidate_tail_entity_type_list = head_with_relations_and_tail_candidates[head_type][_relation]
            ...
            # logger.info("the number of candidate_tails:{0}".format(len(candidate_tail)))
            for corrupt_ent in candidate_tail:
                tmp_triple = [str(head), str(corrupt_ent), str(_relation_id)]
                _tail_corrupt_list.append(tmp_triple)

            logger.info(
                "obtain the number of _tail_corrupt_list:{0} according to the relation : {1} ".format(
                    len(_tail_corrupt_list),
                    _relation))
            _each_relation_candidates.append(len(_tail_corrupt_list))
            tail_corrupt_list += _tail_corrupt_list

        if len(tail_corrupt_list) == 0:
            logger.info(
                "this test_nodes:{0} does not have any issue links, and this is a isolated node\n".format(test_nodes))
            f = open(file_prefix + "_result_of_isolated_node_classification.txt", "a+")
            f.writelines(node_name + "\t" + str(1.0) + '\t' + str(0) + '\t' + str(0) + '\t' + "YES" + '\n')
            f.close()
            continue

        logger.info(
            "total of tail_corrupt_list:{0} for given isolated node : {1}".format(len(tail_corrupt_list), test_nodes))

        mean_each_rel_candidates = np.mean(_each_relation_candidates)
        logger.info(
            "mean of the each relation candidates:{0} for given isolated node : {1}".format(
                int(mean_each_rel_candidates), test_nodes))

        if mean_each_rel_candidates == 0:
            logger.info(
                "this test_nodes:{0} does not have any issue links, and this is a isolated node\n".format(test_nodes))
            f = open(file_prefix + "_result_of_isolated_node_classification.txt", "a+")
            f.writelines(node_name + "\t" + str(1.0) + '\t' + str(0) + '\t' + str(0) + '\t' + "YES" + '\n')
        ...
        tail_corrupt_random_list = random.sample(tail_corrupt_list, int(mean_each_rel_candidates))

        eval_examples = processor.to_create_test_triples_for_isolated_nodes(tail_corrupt_random_list)
        eval_features = processor.to_convert_examples_to_features(
            eval_examples, label_list, args.max_seq_length, tokenizer)
        ...
        for batch in tqdm(eval_dataloader, desc="isolated node detection\n"):
            ...
            with torch.no_grad():
                logits = model.predict(input_ids, segment_ids, input_mask, labels=None)
        ...
        num_pre_ture = 0
        pre_true_negative_triples = []
        pre_true_negative_score = []
        for i in range(len(preds)):
            if preds[i] == 0:
                num_pre_ture += 1
                pre_true_negative_triples.append(tail_corrupt_random_list[i])
                pre_true_negative_score.append(preds_array[i])

        result = metrics.accuracy_score(all_label_ids, preds)

        flag = None
        if result >= args.threshold_value or num_pre_ture == 0:
            flag = "YES"
            pre_labels.append(flag)
        else:
            flag = "NO"
            pre_labels.append(flag)
        logger.info("result is : {0} ".format(result))
        f = open(file_prefix + "_result_of_isolated_node_classification.txt", "a+")
        pre_ture_negative_triple_list_string = "\t".join(str(item) for item in pre_true_negative_triples)
        pre_ture_negative_score_list_string = "\t".join(str(scor) for scor in pre_true_negative_score)
        f.writelines(node_name + "\t" + str(result) + '\t' + str(len(all_label_ids)) + '\t' + str(
            num_pre_ture) + '\t' + flag + '\t' + pre_ture_negative_triple_list_string + '\t' + pre_ture_negative_score_list_string + '\n')
        f.close()

    recall = len([i for i in range(len(pre_labels)) if pre_labels[i] == 'YES']) / len(pre_labels)
    logger.info("================== " + args.data_dir.split('/')[
        -1] + " ==  " + str(args.test_file) + "  ====================\n\n")

    logger.info("recall is : {0} ".format(recall))
    output_eval_file = os.path.join(args.output_dir, "result_of_isolated_node_classification.txt")
    with open(output_eval_file, "a+") as writer:
        writer.write("==================  " + args.data_dir.split('/')[
            -1] + " ==  " + str(args.test_file) + " ====================\n")
        logger.info("***** Test results *****")
...
# python  relation_tail_prediction.py    --task_name kg    --do_relation_tail_prediction   --data_dir ./data/Apache  --model_type  BERT   --model_name_or_path  bert-base-cased   --max_seq_length  400   --per_gpu_train_batch_size   16    --learning_rate 5e-5   --gradient_accumulation_steps 2  --eval_batch_size  512   --pre_process_data  ./pre_process_data   --negative   2   --num_train_epochs   2   --output_dir  ./output_Apache_e2_4_bert_base_max_seq_400/
