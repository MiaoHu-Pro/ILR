...
if __name__ == "__main__":
    ...
    entity2obj = read_enttiy2obj("../data/Apache/ID_Name_Project_Type_Status_sMention_Time.txt")
    row, colum = entity2obj.shape
    print(row, colum)
    length_list = []
    length_less_than_10 = []
    for i in range(row):
        _description = (entity2obj[i][-2])
        if _description is not np.nan :
            s = len(_description.split(" "))
            length_list.append(s)
            if s < 20:
                length_less_than_10.append(s)

    print(np.mean(length_list))
    print(len(length_less_than_10)/1014926)


    # static_frequency("../data/MongoDB/issue_project_frequency.txt",entity_type)
