...
if __name__ == "__main__":
    ...
    for _folder in file_name:
        print(_folder)
        _triples = read_issue_links_with_time("./datasets/" + _folder + "/" + _folder + "_entity_id_time.txt")
        ...
        for i in range(len(_triples)):
            if datetime.strptime(_triples[i][2], format_pattern) <= datetime.strptime(_split_time, format_pattern):
                _train.append(_triples[i])
            else:
                _test.append(_triples[i])

        write_issue2id("./datasets/" + _folder + "/train_entity2id.txt", _train)
        write_issue2id("./datasets/" + _folder + "/test_entity2id.txt", _test)
